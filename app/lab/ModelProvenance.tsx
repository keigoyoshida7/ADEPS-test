...
export default function ModelProvenance({ kind = 'all' }: { kind?: Kind }) {
  ...
  const descriptions = [
    {
      ...
      title: l('拡散スタジオ・小型prior', 'Diffusion studio · small prior'),
  ...
  return <section className="model-provenance" aria-label={l('モデルのデータと学習記録', 'Model data and training provenance')}>
    <div className="mp-heading">
      ...
      <h3>{l('このモデルは何から学んだのか', 'What this model learned from')}</h3>
      <p>{l('学習データは、このプロジェクトのコードで作った合成データです。論文で使用されたデータセット、実際の会場録音、ADEPS公式の学習済み重みは使っていません。画面操作中に再学習する機能はありません。',
        'Training data were generated by this project’s code. The paper’s datasets, real venue recordings and official pretrained ADEPS weights were not used. Operating the interface does not retrain the models.')}</p>
    </div>
    <div className="mp-data-origin">
      <p className="mp-data-status">{l('論文のデータセット：未使用 ／ 本実装：独自の合成データ', 'Paper datasets: not used / This implementation: independently generated synthetic data')}</p>
      <div className="mp-data-comparison">
        ...
        <article><h4>{l('この実装のデータ', 'Data used in this implementation')}</h4>
