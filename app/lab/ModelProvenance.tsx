...
export default function ModelProvenance({ kind = 'all' }: { kind?: Kind }) {
  ...
  return <section className="model-provenance" aria-label={l('モデルのデータと学習記録', 'Model data and training provenance')}>
    <div className="mp-heading">
      ...
      <p>{l('学習データは、このプロジェクトのコードで作った合成データです。実際の会場録音、HARP・VCTK・WSJ0の音声、ADEPS公式の学習済み重みは使っていません。画面操作中に再学習する機能はありません。',
        'Training data were generated by this project’s code. No venue recordings, HARP/VCTK/WSJ0 audio or official pretrained ADEPS weights were used. Operating the interface does not retrain the models.')}</p>
    </div>
