...
const sources = {
  ...
  consistency: { title: 'STFT consistency · Le Roux, Ono & Sagayama', url: 'https://www.jonathanleroux.org/pdf/LeRoux2008SAPA09b.pdf' },
  wiener: { title: 'Consistent Wiener Filtering · Le Roux & Vincent', url: 'https://www.merl.com/publications/TR2012-090' },
  diffpir: { title: 'DiffPIR · Plug-and-Play Image Restoration', url: 'https://arxiv.org/html/2305.08995' },
  bootstrap: { title: 'Bootstrap confidence intervals · SciPy documentation', url: 'https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.bootstrap.html' },
};
const entries: Entry[] = [
  ...
  { term: 'Wiener filtering', jp: '信号と雑音の統計を使って復元する', en: 'Reconstruct using signal and noise statistics', category: 'inference', keywords: 'ウィーナー フィルター 共分散 空間 spatial alpha α',
    ja: '信号と雑音の強さ・成分間の関係を使い、観測をどう組み合わせるか決める方法です。今回のα候補は観測・V・記録された雑音条件から方向成分と広がった成分の統計を求め、Ambisonicsを推定します。正解係数やニューラル学習を使う処理ではなく、統計の仮定が外れると誤差が残ります。Wienerは人名です。',
    explanation: 'Wiener filtering combines observations using signal and noise powers and relationships between components. Our alpha candidate estimates directional and diffuse statistics from observations, V and recorded noise assumptions, then reconstructs Ambisonics. It uses neither target coefficients nor neural training; incorrect statistical assumptions can leave errors. Wiener is a person’s name.', source: 'wiener' },
  { term: 'Proximal correction', jp: '推定を保ちながら観測へ近づける', en: 'Move toward observations while retaining the estimate', category: 'inference', keywords: '近接補正 近接写像 proximal data consistency residual λ lambda alpha α',
    ja: '現在の推定zをVで観測へ戻した値と、入力pとの差を補います。今回の式はa=z+Vᴴ(VVᴴ+λI)⁻¹(p−Vz)。λは観測へ合わせる強さと雑音増幅の折り合いを調整します。圧縮を戻した係数で計算し、正解は使いません。観測に合っても、観測から決まらない成分まで正しいとは限りません。',
    explanation: 'This corrects the difference between input p and the observation predicted from the current estimate z. Here a=z+Vᴴ(VVᴴ+lambda I)⁻¹(p−Vz), with lambda balancing data fit and noise amplification. It operates on uncompressed coefficients without ground truth. Matching observations does not establish correctness of components the observations cannot determine.', source: 'diffpir' },
  { term: 'PnP', names: [{ short: 'PnP', english: 'Plug-and-Play', japanese: '学習済みノイズ除去器を反復復元へ組み込む枠組み', source: 'diffpir' }], jp: '固定したモデルと観測補正を交互に使う', en: 'Alternate a fixed denoiser and observation correction', category: 'inference', keywords: 'プラグアンドプレイ continuation alpha α',
    ja: '今回のα候補では、学習済みDθによる推定と、Vを使う物理的な観測補正を交互に行います。重みは更新せず、σを下げながら推定を整えます。これは独立したPnPの反復法で、原著ADEPSそのものや厳密な事後分布からのサンプルではありません。参照論文のDiffPIRとも同一の実装ではありません。',
    explanation: 'Our alpha candidate alternates a frozen denoiser with physical observation correction through V, reducing sigma during refinement. Weights remain fixed. This independent PnP continuation is neither the original ADEPS algorithm nor an exact posterior sample, and it does not reproduce the cited DiffPIR implementation.' },
  ...
  { term: 'Steps / NFE / saved stages', names: [{ short: 'NFE', english: 'Number of Function Evaluations', japanese: '関数の評価回数（この比較ではノイズ除去ネットワークの呼び出し回数）', source: 'edm' }], jp: '更新回数・モデルの呼び出し回数・途中の推定', en: 'Updates, model calls and saved intermediate estimates', category: 'inference', keywords: 'ステップ checkpoint snapshot チェックポイント 計算量',
    ja: 'Stepsは反復更新の回数、NFEはDθを呼び出した回数です。方式によって一致しないため、計算量の比較では区別します。NFEが少なくても実行時間や精度が必ず良いとは限りません。途中音の連続試聴は保存済み推定の順番再生で、全更新点に音声が保存されているわけではありません。',
    explanation: 'Steps count updates; NFE counts denoiser calls. They can differ between methods, so distinguish them when comparing computation. Fewer calls do not guarantee lower runtime or better accuracy. Process audition plays saved estimates sequentially; not every update has saved audio.' },
  ...
  { term: 'STFT consistency', jp: 'ひとつの波形として成立する係数へ整える', en: 'Make coefficients consistent with a single waveform', category: 'inference', keywords: 'STFT整合性 一貫性 consistency projection 射影 alpha α',
    ja: '時間窓が重なるSTFTでは、各点の係数を自由に変えると、ひとつの波形に戻して同じ係数を得られないことがあります。今回の候補はiSTFTで波形に戻し、同じ窓・間隔でSTFTを取り直して整えます。学習を使わない処理です。波形として成立することと、元の音やマイク観測に正しく合うことは別に評価します。',
    explanation: 'Overlapping STFT windows constrain which coefficient arrays can represent one waveform. Independently edited coefficients may not survive waveform reconstruction and reanalysis. Our candidate applies iSTFT then STFT with matching windows and hop. This requires no training; waveform consistency is evaluated separately from accuracy and agreement with microphone observations.', source: 'consistency' },
  ...
  { term: 'Ablation', jp: '要素を外して、その役割を比べる', en: 'Remove a component to examine its contribution', category: 'evaluation', keywords: 'アブレーション 要素 分解 比較 alpha α no denoiser',
    ja: '改善案から一部の処理を外し、同じ入力で結果がどう変わるか比べます。今回ならノイズ除去器を外した版や、空間推定だけの版も比較対象です。全体がLinearより良くても、それだけでは学習モデルが効いた証拠にならないため、各要素の寄与を分けて確認します。',
    explanation: 'An ablation removes part of a method and compares results on the same input. Here examples include removing the denoiser or using spatial estimation alone. Improvement over Linear by the full system does not by itself establish a neural-model contribution; component comparisons help identify it.' },
  { term: 'CI / cluster bootstrap', names: [{ short: 'CI', english: 'Confidence Interval', japanese: '信頼区間', source: 'bootstrap' }], jp: '比較結果の不確かさを幅で示す', en: 'Express uncertainty in the comparison as an interval', category: 'evaluation', keywords: '信頼区間 ブートストラップ クラスター 話者 統計 confidence interval alpha α',
    ja: '同じシーンでの改善量を求め、話者ペアごとにまとめて再抽出し、平均のばらつきを推定します。今回の計画は4つの話者ペアを単位とする97.5%の区間です。全256通りを計算しても、元の独立な組数は4のまま。少数データなので区間の信頼性には限界があり、すべての音源や実空間で改善する保証ではありません。',
    explanation: 'Paired scene improvements are resampled by speaker-pair group to estimate uncertainty in their mean. This protocol uses a nominal 97.5% interval from four groups. Enumerating all 256 resamples still leaves only four original independent groups. With so few groups, interval coverage is uncertain; it cannot guarantee improvement across all sources or real rooms.' },
  ...
  { term: '3D directional RMS / covariance', names: [{"short": "RMS", "english": "Root Mean Square", "japanese": "二乗平均平方根", "source": "rms"}, {"short": "3D", "english": "Three-dimensional", "japanese": "3次元", "kind": "notation"}], jp: '成分間の関係から、方向ごとの強さを描く', en: 'Use relationships between components to show directional strength', category: 'evaluation', keywords: '共分散 二次モーメント クロスパワー shape 形 確率 音圧 source map',
    ja: '共分散は成分がどう一緒に変わるかを表します。このUIのcovarianceは平均を引かないクロスパワー（二次モーメント）で、成分ごとの強さも含みます。FOAの行列と方向の基底から3Dの大きさを計算します。距離を持つ部屋の音圧や音源位置の地図ではなく、形の変化だけでは精度改善を示しません。',
    explanation: 'Covariance describes how components vary together. The UI’s covariance field is an uncentered cross-power matrix (second moment), which also includes each component’s power. Its FOA matrix and directional basis produce the 3D magnitude. This is not room pressure over distance or a source-location map; a shape change alone does not establish better accuracy.' },
