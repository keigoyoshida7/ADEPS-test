...
**論文の拡散推論式と、独自学習の小型ニューラルpriorを実装しました。** 合成データ・複素STFT・短いマイク録音から、線形との比較とFOA WAV保存ができます。著者の公式モデル・学習済み重みは未公開で、論文の性能を再現したものではありません。[ニューラル実験の使い方と条件](docs/NEURAL_PROTOCOL.md)を参照してください。手法の対応と未実装範囲はUIの「論文と実装の範囲」、[読解ノート](docs/ADEPS_REVIEW.md)に記載しています。
...
4. ニューラル推論では合成データを実行し、線形との比較と反復履歴を確認できます。「合成WAV例を選ぶ」でFOA WAV書き出しも試せます。
5. マイク画面では、同一平面の配置にしてFOA復元のrank低下を確認できます。
6. IR画面では合成サンプルZIPを読み込んでから、自分で収録したIRの入力形式を確認できます。
7. JSON・CSVで計算条件と結果を保存できます。表示言語を切り替えても数値は変わりません。
...
| ニューラル推論 | 独自学習MLP、論文に基づく拡散推論式、線形との比較、反復履歴、短い録音のFOA WAV出力 |
