...
この文書は、同梱モデルカード、学習コード、重みファイルを照合した記録です。**現在の学習データは、このプロジェクトのコードで生成した合成データです。論文のデータセットは使用していません。** 実際の会場録音、測定した室内IR、HARPで生成したIR、VCTK・WSJ0の音声コーパス、ADEPS著者の学習済み重みは使用していません。今回の説明追加で再学習は行っていません。
...
| 画面で聴く音 | `backend/diffusion_studio.py` が生成する短いチャープ・トーン・ノイズ。論文の音声サンプルやVCTK/WSJ0の抜粋ではない |
...
**English — paper citation is not dataset reuse.** The paper reports HARP-generated Ambisonics room IRs convolved with VCTK speech for training and WSJ0 speech for evaluation (20,000 training and 1,000 evaluation scenes). These data were **not used** to train the bundled models. TinyDenoiser uses 48,000 independently generated coefficient vectors and 2,048 validation vectors. The v0.5.3 denoiser test uses 2,048 further vectors from the same procedural family with separate seeds. Studio preview sounds are generated chirps, tones and noise, not excerpts from the paper or speech corpora. We cite the paper's concepts and equations; this does not imply reuse of its datasets, official weights or reproduction of its scores. Sources: [ADEPS §4](https://arxiv.org/html/2608.24558v3#S4) and [§3](https://arxiv.org/html/2608.24558v3#S3).
...
This document checks the bundled model cards, training scripts and weight files. **Training data were generated by this project’s code; the paper's datasets were not used.** No venue recordings, measured room IRs, HARP-generated IRs, VCTK/WSJ0 speech corpora or pretrained weights from the ADEPS authors were used. Documenting provenance did not retrain either model.
