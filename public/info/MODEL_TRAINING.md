# 旧モデルのデータと学習記録 / Archived model data and training record

この文書は旧モデルの来歴を残すアーカイブです。現在の公開UIは`paper-prior-v1`を対象とし、[音声priorの実装と評価](PAPER_PRIOR.md)を参照してください。以下の記録は、`tiny-spatial-v1`と`spatial-v1`を対象とします。**この2モデルは、このプロジェクトのコードで生成した合成データを使っています。** 実際の会場録音、測定した室内IR、HARPで生成したIR、VCTK・WSJ0の音声コーパス、ADEPS著者の学習済み重みは、この2モデルの学習には使用していません。

新しくローカルで学習する音声・時間周波数priorは、別のモデルと別の重みです。[音声priorの実装と評価](PAPER_PRIOR.md)と[保存された学習・評価記録](../public/models/paper-prior-training.json)を参照してください。公開ページでは新モデルの記録と計算済み例を表示し、旧モデルの操作・評価欄は取り除いています。新モデルの学習状況や精度は、実際に保存された記録の範囲で確認します。

## 論文から何を参照し、どのデータを使ったか

| 対象 | データの出所・実装での扱い |
|---|---|
| ADEPS論文の学習・評価 | HARPで合成したAmbisonics室内IRと音声を畳み込み。学習の音声はVCTK、評価はWSJ0。学習20,000シーン、評価1,000シーンと記載。下記の独自データとは異なる |
| 同梱TinyDenoiserの学習 | このプロジェクトの `train_tiny_prior.py` で48,000個の空間係数ベクトルを生成。検証2,048個。声・音楽のコーパスや室内IRを使わない |
| v0.5.3の除去器評価 | 同じ合成規則から別seedで生成した2,048ベクトル。各σで同じ正解とノイズを使う。学習・元の検証と別の標本だが、別の実環境での評価ではない |
| 旧ブラウザー版の仮想マイク実験で聴く音 | `backend/diffusion_studio.py` が生成する短いチャープ・トーン・ノイズ。論文の音声サンプルやVCTK/WSJ0の抜粋ではない。別モデルの保存済み音声評価が追加された場合は、その出典を別に記録する |
| 新しい音声prior | ローカルTorch用の別モデル。データ、話者分割、実際に学習したシーン、更新回数、重みSHA-256、独立評価は[専用記録](../public/models/paper-prior-training.json)に保存。既存2モデルの学習来歴と混ぜない |
| 論文から参照した部分 | アレイと独立した係数prior、圧縮表現、観測整合を伴う拡散推論、ρで離散化するノイズスケジュールなどの考え方と式。引用はデータセットや公式重みの使用を意味しない |

論文側の記述は[ADEPS v3 §4 Numerical Study](https://arxiv.org/html/2608.24558v3#S4)、手法・式は[§3](https://arxiv.org/html/2608.24558v3#S3)を参照しています。HARPはIRの生成・データの枠組みであり、VCTK/WSJ0のような発話コーパスと同じものではありません。現在の実装の独自データ生成条件は以下と[コード](https://github.com/keigoyoshida7/ADEPS-test/blob/main/scripts/train_tiny_prior.py)、追加評価は[検証記録](../public/models/tiny-denoiser-evaluation.json)で確認できます。論文の評価条件と揃っていないため、今回のNRMSEを論文の表のスコアへ直接並べて優劣を判断しません。

**English — model records are separate.** The paper reports HARP-generated Ambisonics room IRs convolved with VCTK speech for training and WSJ0 speech for evaluation (20,000 training and 1,000 evaluation scenes). These data were **not used** to train the two existing browser models, `tiny-spatial-v1` and `spatial-v1`. TinyDenoiser uses 48,000 independently generated coefficient vectors and 2,048 validation vectors. The v0.5.3 denoiser test uses 2,048 further vectors from the same procedural family with separate seeds. The archived browser Studio used generated chirps, tones and noise; the current interface uses the full-size speech prior and its separately attributed examples. The new local Torch speech prior has its own [implementation record](PAPER_PRIOR.md) and [training/evaluation JSON](../public/models/paper-prior-training.json). Its actual corpus use and results must be read there, not inferred from the older records. Similar concepts or parameter counts do not establish use of official weights or reproduction of the paper's scores. Sources: [ADEPS §4](https://arxiv.org/html/2608.24558v3#S4) and [§3](https://arxiv.org/html/2608.24558v3#S3).
...
The following sections check the two existing browser model cards, training scripts and weight files. **Their training data were generated by this project’s code; the paper's datasets were not used for these two models.** Neither used venue recordings, measured room IRs, HARP-generated IRs, VCTK/WSJ0 speech corpora or pretrained weights from the ADEPS authors. The new local Torch speech prior has separate data, weights and [training records](PAPER_PRIOR.md).
